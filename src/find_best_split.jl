...
function _find_best_split(
    loss::LogitLogLoss,
    feature::AbstractVector,
    target::SubArray{A,B,C,D,E},
    warmstart::AbstractVector,
    lambda::Number,
    gamma::Number;
    kwargs...,
) where {A,B,C<:CategoricalArray,D,E}
    @assert length(levels(target)) == 2

    find_best_split(
        loss,
        feature,
        mappedarray(x -> 2 - x.level, target),
        warmstart,
        lambda,
        gamma;
        kwargs...,
...
function _find_best_split(
    ...
    for (i, (f, cg, ch)) in
        enumerate(zip(drop(feature, 1), @view(cg[1:end-1]), @view(ch[1:end-1])))
        if f != last_feature
    ...
    (
        ...
        lweight = lweight,
        rweight = rweight,
        should_split_further = should_split_further, missing_go_left = true,
    )
end
