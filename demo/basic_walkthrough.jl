...
const DATAPATH = "c:/data/agaricus/"
...
# JLBoost supports DataFrames
using DataFrames
df = DataFrame(hcat(train_X, train_Y))
...
using XGBoost
@time bst = xgboost(train_X, num_round, label = train_Y, eta = 1, max_depth = 2,
               objective = "binary:logistic")

# so much slower than
@time bstjl = jlboost!(df, :x127, [Symbol("x"*string(i)) for i in 1:126]; nrounds = 2, eta = 1, max_depth = 2, verbose=true)

@time JLBoost.fit_tree!(df, :x127, [Symbol("x"*string(i)) for i in 1:126]; max_depth = 1, verbose=false)

@time best_split(JLBoost.logloss, df[!, :x29], df[!, :x127], df[!, :prev_w], 0, 0)

@code_warntype best_split(JLBoost.logloss, df[!, :x29], df[!, :x127], df[!, :prev_w], 0, 0)

using BenchmarkTools
@benchmark best_split(JLBoost.logloss, df[!, :x29], df[!, :x127], df[!, :prev_w], 0, 0)

@benchmark predict(bstjl, df)
...
preds = XGBoost.predict(bst, test_X)
...
preds2 = XGBoost.predict(bst2, test_X)
